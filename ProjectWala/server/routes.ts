...
apiRouter.post('/auth/admin-login', async (req, res) => {
  try {
    ...
    if (password === '60212724') {
      const token = jwt.sign({ id: 'admin_id', role: 'Admin' }, process.env.JWT_SECRET || 'fallback_secret', { expiresIn: '30d' });
      res.json({ _id: 'admin_id', name: 'Administrator', email: 'admin@system.local', role: 'Admin', token });
    } else {
